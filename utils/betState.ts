...
import { currentUnixTime, sum } from "./helpers";
...
export const BetState = {
  ...
  Active_Closed_ResolvableNever: "Active_Closed_ResolvableNever",
...
export const handleSetBetState = (virtualFloor: VirtualFloorEntity): BetState => {
  ...
  switch (virtualFloor.state) {
    case VirtualFloorEntityState.Active_ResultNone: {
      // The number of outcomes that have > 0 commitments to them
      const nonzeroOutcomeCount = sum(virtualFloor.outcomes.map(({ totalSupply }) => new BigDecimal(totalSupply).eq(0) ? 0 : 1))
      assert(virtualFloor.bonusAmount !== undefined);
      const nonzeroOutcomeCountCorrectedWithBonus = nonzeroOutcomeCount + (new BigDecimal(virtualFloor.bonusAmount).eq(0) ? 0 : 1);
      ...
      if (t < tClose) {
        ...
        if (nonzeroOutcomeCountCorrectedWithBonus >= 2) {
          if (t >= tClose && t < tResultSetMin) {
          ...
          resp = { uiState: BetState.Active_Closed_ResolvableNever, reason: "Betting is closed, but bet is unresolvable because there weren’t bets on at least two outcomes.", status: "Unresolved" };
        }
      }
      break;
    }
    ...
    case VirtualFloorEntityState.Claimable_Refunds_Flagged:
      // ToDo: Replace with:
      // resp = { uiState: BetState.Claimable, reason: `Bet was cancelled because it was flagged for the following reason: ${virtualFloor.flaggingReason}`, status: "Cancelled" };
    ...
    case VirtualFloorEntityState.Claimable_Refunds_ResolvableNever:
      ...
      resp = { uiState: BetState.Claimable, reason: "Bet was cancelled because there weren’t bets on at least two outcomes", status: "Cancelled" };
      break;
