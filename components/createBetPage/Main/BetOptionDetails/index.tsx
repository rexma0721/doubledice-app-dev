...
const BetOptionDetails = ({ nextStep }: PropsI) => {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // ToDo: Replace with `const currencyNames = paymentTokens.map(({ name }) => name);`
        const currencyNames: string[] = [];

        // ToDo: Simply use paymentTokens directly
        ...
        paymentTokens.forEach((paymentToken: PaymentToken) => {
          ...
          currencies.push({
            ...
            id: paymentToken.id,
            isWhitelisted: paymentToken.isWhitelisted,
            isTest: paymentToken.isTest,
          });
        });
        ...
        if (error instanceof Error) {
          Honeybadger.notify(`Fetching payment method failed with: ${error}`);
        }
      }
    })();
  }, []);
    ...
    if (e.target && e.target.files && e.target.files[0]) {
      ...
      if (size / 1000 <= 200) {
        try {
          ...
          Honeybadger.notify(`Converting image to base64 failed with: ${error}`)
        }
  ...
  const toggleNextStep = () => {
    ...
    const chosenOpponent = opponents.map((opponent) => ({
      title: opponent.title.trim(),
      image: opponent.image.trim(),
    }));

    dispatch(
      setFormData({
        opponents: chosenOpponent,
        outcomes,
        paymentToken,
